	...
	buildForm(div,tab){
		...
		for(var i=0;i<conf.fields.length;i++){
			...
			if(!fldconf.isprikey){
				...
				if(fldconf.fieldname=="applicationid")conf.default.applicationid=NUT.ctx.curApp.applicationid;
			}		
			var domain=null;
		...
		var items=tab.windowsearch?[{type:'button',id:"OK",text:'✔️',tooltip:"Ok"},
					{type:'break'},
					{type:'button',id:"CLEAR",text:'🧹 Clear',tooltip:"Clear"},
					{type:'button',id:"SEARCH",text:'🔍 Search',tooltip:"Search"},
					{type:'break'}]:
					[{type:'button',id:"GRID",text:'⧉',tooltip:"Form/Grid"},
					{type:'break'},
		...
		if(!tab.windowsearch){
			...
			for(var i=0;i<conf.menuitems.length;i++){
				...
				var item={type:(isSummary?'menu':'button'),id:menuitem.execname?menuitem.execname:menuitem.menuitemid,text:menuitem.menuitemname,tooltip:menuitem.description,tag:menuitem};
