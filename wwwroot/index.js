		...
		if (res.success) {
			if (res.results.length == 1) {
				...
				var mnuMain = new Smart.Menu("#divMenu", {
					dataSource: [
						{ value:'start', label: '<table><tr><td rowspan="2"><img src="site/' + NUT.ctx.user.siteid + '/logo.png"/></td><td><b>' + NUT.ctx.user.sitecode + '</b></td></tr><tr><td>' + NUT.ctx.user.sitename + '</td></tr></table>' },
						{
			...
			if (res.success) {
				...
				NUT.ds.select({ from: "n_menu", order: "seqno", where: ["or", ["menuid", "in", Object.keys(ids)], ["menuid", "in", Object.keys(pids)]] }, function (res2) {
					if (res2.success) {
						...
						window.mnuApp = new Smart.Menu("#divLeft", {
							mode : "dropDown",
							dataSource: nodes
						});
						window.mnuApp.addEventListener("itemClick", mnuApp_click);
