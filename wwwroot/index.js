...
window.openApp=function(id){
	...
	if(n$.app.apptype=="engine"){
		...
		//load service
		NUT.ds.select({ url: NUT.URL + "nv_appservice_service",siteid:n$.user.siteid,where:["appid","=",id]},function(res){
			NUT.urls = {};
			if (res.success)
				for (var i = 0; i < res.result.length; i++) {
					var service = res.result[i];
					NUT.urls[service.serviceid] = service.url;
				}
			else
				NUT.notify("⛔ ERROR: " + res.result, "red");
		});
