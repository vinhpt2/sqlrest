	...
	runReport:function(){
		if(numTinhCong_Year.value&&numTinhCong_Month.value){
			...
			NUT_DS.select({url:url+"phucap",where:[["nam","=",numTinhCong_Year.value],["thang","=",numTinhCong_Month.value],["vitrilv","=","PG"]]},function(phucaps){
				if(phucaps.length)NUT_DS.select({url:url+"rpt_weeklysanluong",where:[["nam","=",numTinhCong_Year.value],["thang","=",numTinhCong_Month.value],["madoitac","=",cboTinhCong_DoiTac.value],["dulieu","=",chkTinhCong_Edit.checked?1:0]]},function(res){
					if(res.length){
						var pcPG=phucaps[0];
