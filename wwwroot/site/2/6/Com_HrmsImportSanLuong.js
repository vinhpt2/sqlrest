var Com_HrmsImportSanLuong={
	run:function(p){
		...
		w2popup.open({
			...
			width: 1000,
			height: 750,
			body: "<textarea cols='"+(header.length+8*fieldnames.length)+"' id='"+id+"' style='height:95%'>"+header+"</textarea><table style='margin:auto'><tr><td><label for='chkTinhCong_SanLuongTuan'>Sản lượng tuần/két</label></td><td><input id='chkTinhCong_SanLuongTuan' type='checkbox'/></td><td>Đối tác</td><td colspan='3'>"+cbo.outerHTML+"</td><td>Năm</td><td><input id='numTinhCong_Year' style='width:60px' type='number'/></td><td>Tháng</td><td><input id='numTinhCong_Month' style='width:60px' type='number'/></td><td><input id='chkTinhCong_Edit' type='checkbox' checked/></td><td><label for='chkTinhCong_Edit'>Ghi vào dữ liệu Hiệu chỉnh</label></td><td><b style='color:red'>(Dữ liệu CŨ sẽ bị ghi đè!)</b></td></tr></table>",
					...
					for(var j=0;j<cols.length;j++){
						if(line0[j].includes("-")){
							...
							if(ngay!=oldNgay){
								...
								oldNgay=ngay;
								json={madoitac:madoitac,manhanvien:cols[header.manhanvien],madiemban:cols[header.madiemban],nam:numTinhCong_Year.value,thang:numTinhCong_Month.value,dulieu:dulieu};
								chkTinhCong_SanLuongTuan.checked?json.tuan=ngay:json.ngay=ngay;
							}
